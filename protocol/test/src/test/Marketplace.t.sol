...
contract MarketplaceTest is BaseTest {
    ...
    function test_offer_bidAuctionNativeToken() public {
        ...
        vm.warp(1);
        vm.prank(getActor(0));
        marketplace.offer{value: 1 ether}(
            listingId,
            1,
            NATIVE_TOKEN,
            1 ether,
        ...
        vm.warp(2);
        vm.prank(getActor(0));
        marketplace.offer{value: 2 ether}(
            listingId,
            1,
            NATIVE_TOKEN,
            2 ether,
    ...
    function test_closeAuctionForCreator_afterBuyout() public {
        ...
        vm.warp(1);
        vm.prank(getActor(1));
        marketplace.offer{value: 5 ether}(
            listingId,
            1,
            NATIVE_TOKEN,
            5 ether,
        ...
        vm.warp(2);
        vm.prank(getActor(2));
        marketplace.closeAuction(listingId);

        uint256 listerBalAfter = getActor(0).balance;
        uint256 winningBidPostFee = (5 ether * (MAX_BPS - marketFeeBps)) /
            MAX_BPS;
    ...
    function test_acceptOffer_whenListingAcceptsNativeToken() public {
        ...
        uint256 offerValuePostFee = (4 ether * (MAX_BPS - marketFeeBps)) /
            MAX_BPS;

        vm.prank(getActor(0));
        marketplace.acceptOffer(listingId, getActor(1));
        assertEq(erc721.ownerOf(listing.tokenId), getActor(1));
